...
# 2. 오디오 파일을 10분 단위로 나누기 (청크 간 오버랩 방지 수정)
def split_audio(file_path, chunk_length_ms=10 * 60 * 1000):
    ...
    for i in range(0, len(audio), chunk_length_ms):
        chunks.append(audio[i : i + chunk_length_ms])  # 정확히 분리 (오버랩 방지)
...
# 반복 텍스트 제거 함수 추가 (제안된 수정)
def remove_repeated_sentences(transcription):
    """
    Removes repeated sentences from the transcription.
    """
...
def process_large_audio(file_path, output_directory):
    ...
    # 반복 텍스트 제거 적용 (제안된 수정)
    full_transcript = remove_repeated_sentences(full_transcript)
